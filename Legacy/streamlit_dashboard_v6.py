...
#         annual_columns = ["Year", "Installed Capacity (MW)", "Generation (GWh)", "Imports (GWh)", "Consumption (GWh)"]
#         sector_columns = ["Year", "Domestic", "Commercial", "Industrial", "Agriculture", "Street Light", "Traction", "Bulk Supply", "other Govt", "Total"]

#         if all(col in data.columns for col in annual_columns) or all(col in data.columns for col in sector_columns):
#             return True, None
#         return False, "The uploaded file is missing required columns for both datasets."
#     except Exception as e:
#         return False, f"An error occurred while processing the file: {e}"
...
def process_uploaded_file(uploaded_file):
    ...
    if uploaded_file:
        try:
            data = pd.read_excel(uploaded_file)
            missing_columns_annual = [col for col in annual_columns if col not in data.columns]
            missing_columns_sector = [col for col in sector_columns if col not in data.columns]
            if not missing_columns_annual or not missing_columns_sector:
                return True
            return False
...
with st.sidebar:
    ...
    # Python button actions
    col1, col2 = st.columns(2)

    with col1:
        if st.button("Logout", key="logout_button"):
            os.system("streamlit run streamlit_dashboard_login_page_v2.py")

    with col2:
        if st.button("Contact Us", key="contact_us_button"):
            os.system("streamlit run streamlit_dashboard_contact_us_page.py")
basis_selection=""  
# invalidfile=False
if uploaded_file:
    is_valid = process_uploaded_file(uploaded_file)
    if is_valid:
        data = pd.read_excel(uploaded_file)
        ...
        if numeric_columns:
            ...
            if basis_selection:
                st.write(f"You selected: {basis_selection}")
        else:
            invalidfile=True
            st.warning("No numeric columns found in the uploaded file.")
else:
        invalidfile=True
        st.warning("Invalid data uploaded")
        # uploaded_file = None
...
# Main Content Area
if uploaded_file and basis_selection and -invalidfile:
